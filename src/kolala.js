...
koala.loadImage = function(imageData) {
  ...
  canvas.drawImage(imageData, 0, 0, dim, 64);
  ...
  return canvas.getImageData(0, 0, dim, 64).data;
};
...
koala.makeCircles = function(selector, colorData) {
  ...
  for (yi = 0; yi < dim; yi++) {
    for (xi = 0; xi < dim; xi++) {
      ...
      firstLayer(xi, yi, new Circle(vis, xi, yi, size, color));
      t += 4;
    }
  }
  debugger;
  ...
  while (size < maxSize) {
    ...
    for (yi = 0; yi < dim; yi++) {
      for (xi = 0; xi < dim; xi++) {
        ...
        c1.parent = c2.parent = c3.parent = c4.parent = layer(
          xi,
          yi,
          new Circle(
            vis,
